...
void skipComment() {
  do{
    ...
    readChar();
  }while(currentChar != EOF && charCodes[currentChar] != CHAR_RPAR);
  if(currentChar == EOF){
    error(ERR_ENDOFCOMMENT, lineNo, colNo);
  }else{
    readChar();
...
Token* readIdentKeyword(void) {
  ...
  // in the case user enter an ident > 15 characters
  // the last character (16) will not be '\0'
  // because token->string has been declared [MAX_IDENT_LEN + 1] only
  // how to return the TK_IDENT in this case
  // solution: increase sizeof token->string 
  // another solution: save the ident in buffer string to check before assigning to token->string
